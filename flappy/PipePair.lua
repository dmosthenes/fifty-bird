...
function PipePair:init(y)
    ...
    -- clamp pipes to avoid sprite cut-off
    self.pipes = {
        ['upper'] = Pipe('top', math.max(-240, self.y)),
        ['lower'] = Pipe('bottom', math.min(VIRTUAL_HEIGHT - PIPE_HEIGHT/4, self.y + PIPE_HEIGHT + GAP_HEIGHT))
    }
